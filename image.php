...
if(isset($_GET['jid']))
{
	...
	$uname = str_replace('@xmpp.xxxxxx.com','',$nstr);
	$uname = str_replace('@server.fwends.co.uk','',$str);
	// connect to db
	$link = dbconnect();
	$sql= mysql_query('SELECT `id`,`gender` FROM `users` WHERE `nickname` LIKE "'.$uname.'"');
	if(mysql_num_rows($sql) > 0)
	{
		$uid = mysql_result($sql,0,'id');
		$gen = mysql_result($sql,0,'gender');
		$sql = mysql_query('SELECT `path` FROM `images` WHERE `uid` = "'.$uid.'" AND `ismain` = "1"');
		if(mysql_num_rows($sql) > 0)
		{
