...
$images_path = "/home/www/xxxxxx.com/htdocs/images/p_pics/";  //replace this with the path where your images are stored, relative to the directory this script will run from
$thumbs_path = "/home/www/xxxxxx.com/htdocs/images/thumbs/"; //this is you destination thumbmail folder, you can change it if you want, just be sure the folder actually exists and is writeable by the web server.  In this example, we don't deal with duplicate names, so we just store the cropped verion in a different folder.
...
//{
	//$msg .= $key . " : " . $value . "\n";
//}
//mail('xxxxxx@gmail.com','test','post vars: ' . $msg);

// get photo id for update
function db_connect()
{
...
$link = db_connect();
$sql = mysql_query('SELECT `id` FROM `images` WHERE `path` = "/images/p_pics/'.$photo.'"');
$imageid = mysql_result($sql,0,'id');
//mail('xxxxxx@gmail.com','imgtest',$imageid);
...
//mail('xxxxxx@gmail.com','test','img name / path = '.$thumb);
// update db with thumb
$sql = mysql_query('UPDATE `images` SET `thumb` = "/images/thumbs/'.$thumb.'" WHERE `id` = "'.$imageid.'"');
die('{"success":"'.$thumb.'"}');
?>
